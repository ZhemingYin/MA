...
from models.swinir_tensorflow import SwinTransformer, PatchExtract, PatchReconstruct
...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self, kernel_size=3, filters=64, strides=[1, 1], processing_type='padding', condition1=False, condition2=False, condition3=False):
        super(Encoder, self).__init__()
        self.processing_type = processing_type
        if condition1:
            self.range_preprocessing = tf.keras.layers.SeparableConv2D(filters=8, kernel_size=(2, 1), strides=(1, 1), padding='valid')
        else:
            self.range_preprocessing = tf.keras.layers.Conv2D(8, kernel_size=(2, 1), strides=(1, 1), padding='valid')

        # input convolution
        if condition1:
            self.input_conv1 = tf.keras.layers.Conv2D(filters=filters,
...
def point_wise_feed_forward_network(d_model, dff, condition10=False):
    if condition10:
...
class TransformerSeparator(tf.keras.layers.Layer):
    def __init__(self, num_dp_layers, num_layers,
                 ...
                 window_size=2,
                 qkv_bias=True, dropout_rate=0.03,
        ...
        if not condition5:
            ...
            self.input_conv.add(tf.keras.layers.Conv2D(filters=filters, kernel_size=1, strides=1, activation=None, data_format='channels_last', padding='same'))
            self.input_conv.add(tf.keras.layers.LayerNormalization())

        self.model = keras.models.Sequential()
        # stack of dual-path transformer blocks
        for i in range(num_dp_layers):
            if condition12:
                self.model.add(PatchExtract(patch_size=(2,2)))
                self.model.add(SwinTransformer(
                    dim=d_model,
                    num_patch=(65, 16),
                    x_size= (130, 32),
                    num_heads=num_heads,
                ...
                self.model.add(PatchReconstruct(patch_size=(2,2), original_size=(8, 130, 32, 96)))
...
class TransformerEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, rate=0.1, condition10=False, condition11=False):
        super(TransformerEncoderLayer, self).__init__()
        self.mha = MultiHeadAttention(d_model, num_heads)
        self.ffn = point_wise_feed_forward_network(d_model, dff, condition10=condition10)
    ...
    def call(self, x, training, mask):
        if self.condition11:
            ...
            attn_output, _ = self.mha(out1, out1, out1, mask)
            out2 = self.layernorm2(x + attn_output)
            ffn_output = self.ffn(out2)  # (batch_size, input_seq_len, d_model)
            ffn_output = self.dropout1(ffn_output, training=training)
            ffn_output = self.dropout2(ffn_output, training=training)
            out2 = attn_output + ffn_output
        else:
...
class DPTransformerBase(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff,
        ...
        self.enc_intra_layers = [TransformerEncoderLayer(d_model, num_heads, dff, rate, condition10=condition10, condition11=condition11)
                                 for _ in range(num_layers)]
        # inter transformer layers (here same structure as intra layers for
        # simplicity, might change later)
        self.enc_inter_layers = [TransformerEncoderLayer(d_model, num_heads, dff, rate, condition10=condition10, condition11=condition11)
                                 for _ in range(num_layers)]
...
class DPTransformerSTFT_test(tf.keras.Model):
    def __init__(self, conditions, num_dp_layers, num_layers,
                 ...
                 maximum_position_encoding=10000,
                 re_im_split=False,
                 processing_method='padding&shuffle&ap/ph&with_log10&abs_normalization(0,1)&angle_normalization'):
        super(DPTransformerSTFT_test, self).__init__()
        ...
        self.enc = Encoder(kernel_size=enc_kernel_size, filters=dff,
                           ...
                           condition1=conditions[0], condition2=conditions[1], condition3=conditions[2])

        self.pos_drop = tf.keras.layers.Dropout(rate=0.03)
        ...
        self.sep = TransformerSeparator(num_dp_layers=num_dp_layers,
                                        ...
                                        maximum_position_encoding= \
                                            maximum_position_encoding,
                                        re_im_split=re_im_split,
                                        condition5=conditions[4],
                                        condition6=conditions[5],
                                        condition7=conditions[6],
                                        condition8=conditions[7],
                                        condition10=conditions[9],
                                        condition11=conditions[10],
                                        condition12=conditions[11])
        ...
        self.upsampling_ratio = upsampling_ratio
        self.condition4 = conditions[3]
        self.condition9 = conditions[8]

    def call(self, inputs):
        enc = self.enc(inputs)
        if self.condition4:
            enc = self.pos_drop(enc)
        separated = self.sep(enc)
        if self.condition9:
            separated = tf.math.add(separated, enc)
